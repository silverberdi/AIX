...
| Item | Value |
|------|--------|
| **Wave** | Wave 3 — Capture MVP (**in progress** — slice 016 done; slice 017 next) |
...
| **Next target** | slice-017 — Document metadata attachment on `Document` aggregate |
...
- **Slice 004** — `Document.AttachFile` adds primary/supporting file references (domain metadata only); `DocumentFile` value object, `DocumentFileId`, `DocumentFileRole`; attachment rules enforced; `DocumentFileAttached` event; complete documents reject file mutation (13 tests; **25 total** in `AIX.Documents.Tests`)
